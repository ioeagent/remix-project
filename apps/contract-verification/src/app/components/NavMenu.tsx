...
const NavItem: React.FC<NavItemProps> = ({ to, icon, title }) => {
  ...
  return (
    <NavLink
      data-id={`${title}Tab`}
      to={to}
      className={`no-underline flex px-1 py-1 flex-col justify-center ${isActive ? 'bg-light text-dark border-top border-start border-end' : 'bg-transparent border-0'}`}
    >
      <span>
...
export const NavMenu = () => {
  return (
    <nav className="flex medium flex-row w-full bg-body">
