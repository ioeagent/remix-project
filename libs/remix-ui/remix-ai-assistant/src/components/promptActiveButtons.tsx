...
export function PromptActiveButtons(props: PromptActiveButtonsProps) {
  ...
  return (
    <div
      className="flex justify-between gap-4 items-center w-full py-2 items-center px-4"
      style={{
        backgroundColor: props.themeTracker && props.themeTracker?.name.toLowerCase() === 'light' ? '#d9dee8' : '#222336',
      }}
    >
      <button className="btn btn-sm small font-weight-light text-secondary flex justify-center items-center" style={{
      ...
      <div>
        <i className="fas fa-microphone me-4" style={{ color: props.themeTracker && props.themeTracker.name.toLowerCase() === 'light' ? '#1ea2aa' : '#2de7f3' }}></i>
