...
export function TopCards() {
  ...
  const ImportOptions = () => {
    ...
    return (
      <ul
        className="list-unstyled p-4 gap-2 flex flex-col items-start justify-end bg-light position-absolute"
        ref={importOptionRef}
        style={{
          borderRadius: '10px',
          zIndex: 1000,
      ...
      >
        ...
        >
          <i className="me-2 far fa-cube"></i><span className="font-light">Import from IPFS</span>
        </li>
        <li
          className="flex flex-row items-center import-option-item"
          onClick={() => {
        ...
        >
          <i className="me-2 fa-solid fa-link"></i><span className="font-light">Import from HTTPS</span></li>
      </ul>
    )
  }
  ...
  return (
    <div className="title">
      <div className="flex flex-wrap g-3 mb-1" style={{ position: 'relative' }}>
        <div className="w-full md:w-1/2">
          <div
            data-id="create-blank-workspace-topcard"
            className={`explora-topcard flex flex-row items-center bg-light p-4 p-md-4 shadow-sm border-0 h-full ${theme?.name === 'Dark' ? 'text-white-dimmed' : 'text-dark'}`}
            onClick={async () => {
              if (state.manageCategory === 'Template') {
          ...
          >
            ...
            <span className="flex flex-col grow ms-2 ms-md-3">
              <p className="mb-0 font-semibold">Create blank</p>
              <p className="mb-0 font-light text-wrap">{state.manageCategory === 'Template' ? 'Create an empty workspace' : 'Create a blank file'}</p>
            </span>
          </div>
        </div>
        <div className="w-full md:w-1/2">
          ...
          >
            ...
            <span className="flex flex-col grow ms-2 ms-md-3">
              <p className="mb-0 font-semibold">Create with AI</p>
              <p className="mb-0 font-light text-wrap">{state.manageCategory === 'Template' ? 'Generate a workspace with AI' : 'Generate files with AI'}</p>
            </span>
          </div>
        </div>
        <div className="w-full md:w-1/2">
          ...
          >
            ...
            <span className="flex flex-col grow ms-2 ms-md-3">
              <p className="mb-0 font-semibold">Contract Wizard</p>
              <p className="mb-0 font-light text-wrap">{state.manageCategory === 'Template' ? 'Create a new workspace with the OpenZeppelin Wizard' : 'Create a contract file with the OpenZeppelin Wizard'}</p>
            </span>
          </div>
        </div>
        {(!isElectron() || state.manageCategory !== 'Template') && <div className="w-full md:w-1/2">
          <div
            ref={importCardRef}
            data-id="import-project-topcard"
            className={`explora-topcard flex flex-row items-center p-4  shadow-sm import-files border border-light h-full ${theme?.name === 'Dark' ? 'text-white-dimmed' : 'text-dark'}`}
            style={{
          ...
          >
            ...
            <span className="flex flex-col grow ms-2 ms-md-3">
              <p className="mb-0 font-semibold">{state.manageCategory === 'Template' ? 'Import Project' : 'Import Files'}</p>
              <p className="mb-0 font-light text-wrap">{state.manageCategory === 'Template' ? 'Import an existing project' : 'Import existing files'}</p>
            </span>
          </div>
        </div>}
        {importFiles && <ImportOptions />}
        {state.manageCategory === 'Template' && <div className="w-full md:w-1/2">
          <div
          ...
          >
            ...
            <span className="flex flex-col grow ms-2 ms-md-3">
              <p className="mb-0 font-semibold">Git Clone</p>
              <p className="mb-0 font-light text-wrap">Clone a git repository</p>
            </span>
          </div>
        </div>}
      </div>
