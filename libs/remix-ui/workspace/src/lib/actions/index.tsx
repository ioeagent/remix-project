...
const basicWorkspaceInit = async (workspaces: { name: string; isGitRepo: boolean; }[], workspaceProvider) => {
  if (workspaces.length === 0) {
    ...
    cloudStore.setActiveWorkspace('default_workspace')
    ...
    if (workspaces.length > 0) {
      ...
      cloudStore.setActiveWorkspace((workspace || {}).name)
    }
  }
}
...
export const initWorkspace = (filePanelPlugin) => async (reducerDispatch: React.Dispatch<Actions>) => {
  if (filePanelPlugin) {
    ...
    if (params.gist) {
      ...
      cloudStore.setActiveWorkspace('code-sample')
      ...
      cloudStore.setActiveWorkspace('electron')
