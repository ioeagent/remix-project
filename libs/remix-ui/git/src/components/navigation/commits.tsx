...
export const CommitsNavigation = ({ eventKey, activePanel, callback, title, branch, remote, showButtons, ahead, behind }: CommitsNavigationProps) => {
  ...
  return (
    <>
      <div className={`flex justify-between ${activePanel === eventKey ? 'bg-light' : ''} ${ahead || behind? 'text-success':''}`}>
        <span data-id={`commits-panel${ahead?'-ahead':''}${behind?'-behind':''}`} onClick={() => handleClick()} role={'button'} className='nav flex justify-start items-center w-full ms-1'>
          {
          ...
          <label className={`ps-2 nav form-check-label ${ahead || behind? 'text-success':''}`}>{title}</label>
