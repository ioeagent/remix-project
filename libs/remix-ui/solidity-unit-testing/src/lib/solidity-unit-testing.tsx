...
export const SolidityUnitTesting = (props: Record<string, any>) => {
  ...
  const renderContract = (filename: string, contract: string | null, index: number, withoutLabel = false) => {
    ...
    let label
    if (index > -1) {
      const className = 'alert-danger inline-block mb-1 me-1 p-1 failed_' + runningTestFileName
      label = (
        <CustomTooltip placement={'right'} tooltipClasses="whitespace-nowrap" tooltipId="info-recorder" tooltipText={<FormattedMessage id="solidityUnitTesting.tooltipText1" />}>
      ...
      const className = 'alert-success inline-block mb-1 me-1 p-1 passed_' + runningTestFileName
      label = (
        <CustomTooltip placement={'top-end'} tooltipClasses="whitespace-nowrap" tooltipId="info-recorder" tooltipText={<FormattedMessage id="solidityUnitTesting.tooltipText2" />}>
          <div className={className}>
      ...
      if (!test.rendered) {
        ...
        if (test.type === 'testPass') {
          ...
          const testPassCard: ReactElement = (
            ...
            >
              <div className="flex my-1 items-start justify-between">
          ...
          if (!test.assertMethod) {
            const testFailCard1: ReactElement = (
              <div
                className="bg-light mb-2 px-2 testLog flex flex-col text-danger border-0"
              ...
              >
                ...
                <span className="pb-2 break-words">"{test.errMsg}"</span>
              </div>
            )
            ...
            const testFailCard2: ReactElement = (
              ...
              >
                ...
                <div className="flex flex-wrap">
                  ...
                  <div className="mx-1 font-bold">{method}</div>
                  <div>
    ...
    for (const filename of filenames) {
      ...
      if (testSummary && testSummary.filename && !testSummary.rendered) {
        const summaryCard: ReactElement = (
          <div className="flex alert-secondary mb-4 p-4 flex-col">
            <span className="font-bold">
      ...
      <div>
        <div className="flex p-2">
          ...
          <CustomTooltip
            ...
            placement={'top'}
          >
            <a className="btn border no-underline pe-0 flex w-1/2 ms-2" target="__blank" href="https://remix-ide.readthedocs.io/en/latest/unittesting.html#test-directory">
        ...
        <div className="flex p-2">
          <CustomTooltip placement={'top'} tooltipClasses="whitespace-nowrap" tooltipId="info-recorder" tooltipText={runButtonTitle}>
            <button id="runTestsTabRunAction" data-id="testTabRunTestsTabRunAction" className="w-1/2 btn btn-primary" disabled={disableRunButton} onClick={runTests}>
          ...
          >
            <button id="runTestsTabStopAction" data-id="testTabRunTestsTabStopAction" className="w-1/2 ps-2 ms-2 btn btn-secondary" disabled={disableStopButton} onClick={stopTests}>
              <span>
        ...
        <div className="flex items-center ms-2 me-4 pb-2 mt-2 border-bottom form-check">
          <input
            id="checkAllTests"
          ...
          <label
            ...
            className="form-check-label mb-0 ms-1 whitespace-nowrap"
        ...
        <div className="testList ms-2 pe-2 mt-0 border-bottom py-2">
          {testFiles.length
            ? testFiles.map((testFileObj: TestObject, index) => {
              ...
              return (
                <div className="flex items-center form-check" key={index}>
                  <input
                  ...
                  <label
                    data-id="singleTest"
                    id={"id" + elemId}
                    className="singleTestLabel whitespace-nowrap mb-0 form-check-label ms-1 whitespace-nowrap"
                    htmlFor={elemId}
                    style={{ paddingTop: '0.125rem' }}
                  >
        ...
        <div className="items-start flex-col mt-2 mx-4 mb-0">
        ...
        <div className="mx-4 mb-2 pb-6 border-primary" id="solidityUnittestsOutput" data-id="testTabSolidityUnitTestsOutput">
          {testsOutput}
        </div>
      </div>
    </div>
