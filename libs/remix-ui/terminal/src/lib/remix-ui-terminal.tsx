...
export const RemixUiTerminal = (props: RemixUiTerminalProps) => {
  ...
  const handleAutoComplete = () => (
    <div
      className="remix_ui_terminal_popup bg-light ms-6 p-2 position-absolute text-left "
      style={{
        display:
  ...
  const classNameBlock = 'remix_ui_terminal_block px-6 py-1 break-words'
    ...
    ( props.visible &&
      <div style={{ flexGrow: 1 }} className="remix_ui_terminal_panel h-full mb-2" ref={panelRef}>
        <div tabIndex={-1} className="remix_ui_terminal_container flex h-full m-0 flex-col" data-id="terminalContainer">
          {handleAutoComplete()}
          <div className="position-relative flex flex-col-reverse h-full" key={`terminal-view-${showDebuggerCallStack ? 'debug' : 'normal'}`}>
            {showDebuggerCallStack ? (
              ...
              <>
                <div id="journal" className="remix_ui_terminal_journal flex flex-col pt-4 pb-6 px-2 mx-2 me-0" data-id="terminalJournal">
              ...
              terminalState.journalBlocks.map((x, index) => {
                if (x.name === EMPTY_BLOCK) {
                  ...
                  return x.message.map((msg, i) => {
                    ...
                    if (React.isValidElement(msg)) {
                      return (
                        <div className="px-6 block" data-id="block" key={i}>
                ...
                {isOpen && (
                  <div id="terminalCli" data-id="terminalCli" className="remix_ui_terminal_cli position-absolute w-full" onClick={focusinput}>
                    <span className="remix_ui_terminal_prompt blink mx-1 font-bold text-dark">{'>'}</span>
                    <input
                      className="remix_ui_terminal_input ms-1 text-dark break-words border-0"
                      ref={inputEl}
                      spellCheck="false"
