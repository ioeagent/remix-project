...
export const RemixUITerminalMenuButtons = (props: RemixUiTerminalProps) => {
  ...
  return (
    <div className='flex flex-row items-center'>
      <button id="tabOutput" data-id="tabOutput" className={`xtermButton btn btn-sm border-secondary me-2 border ${!xtermState.showOutput ? '' : 'flex btn-secondary'}`} onClick={selectOutput}>
        Output
      </button>
      <button data-id="tabXTerm" id="tabXTerm" className={`xtermButton btn btn-sm border-secondary ${(xtermState.terminalsEnabled || platform === appPlatformTypes.desktop) ? 'block' : 'hidden'} ${xtermState.showOutput ? '' : 'btn-secondary'}`}
