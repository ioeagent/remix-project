...
export class DappManager {
  ...
  /**
   * Creates a DApp inline — writes generated files to a /frontend directory
   * in the CURRENT workspace instead of creating a new workspace.
   * No workspace switching happens at any point.
   */
  ...
  /**
   * Saves AI-generated files into /frontend in the current workspace (inline mode).
   */
  async saveGeneratedFilesInline(pages: Record<string, string>): Promise<void> {
    ...
    for (const [rawFilename, content] of Object.entries(pages)) {
      ...
      // Prefix every file with frontend/
