...
export const TransactionRecordCard = ({ deployment }: { deployment: Transaction }) => {
  ...
  return (
    <div className="contract-card my-2 rounded" style={{ backgroundColor: 'var(--custom-onsurface-layer-2' }}>
      <div className="flex items-center justify-between w-full p-4 whitespace-nowrap truncate overflow-hidden" onClick={handleCardClick}>
      ...
      {isExpanded && transactions.length > 0 && transactions.map((transaction, index) => {
        ...
        return <div key={index} className="list-group-item px-0">
          <div className="flex items-center justify-between w-full p-4 whitespace-nowrap truncate overflow-hidden border-top" onClick={handleCardClick}>
            <div className='flex'>
              { isSuccess ? (
